  ...
  const HeaderBar = ({ onBack }: { onBack: () => void }) => (
    <div className="flex items-center gap-3 mb-5">
      <button onClick={onBack}
        className="w-9 h-9 rounded-xl flex items-center justify-center bg-white/10 active:scale-90 transition-all">
        <ChevronLeft size={18} className="text-white" />
      </button>
      <p className="text-white font-black text-sm">Mise à jour DNS — Paiement</p>
    </div>
  );
  ...
  if (step === "country") {
    return (
      <div className="space-y-3">
        ...
        <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Choisissez votre pays</p>
        <div className="space-y-2.5">
          {COUNTRIES.map(c => (
            <button key={c.code} data-testid={`button-dns-country-${c.code}`}
              onClick={() => { setCountry(c.code); setOperator(""); setStep("operator"); }}
              className="w-full flex items-center justify-between rounded-2xl p-4 active:scale-[0.98] transition-all"
            ...
            return (
              <button key={opCode} data-testid={`button-dns-operator-${opCode}`}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-white font-bold text-sm">{op.full}</span>
                </div>
                <ChevronRight size={16} className="text-slate-500" />
              </button>
            );
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="rounded-2xl p-4 flex items-center gap-3"
          style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}>
          <span className="text-white font-bold text-sm">+{selectedCountry?.prefix}</span>
          <input
            data-testid="input-dns-phone"
            type="tel" inputMode="numeric" placeholder={phonePlaceholder} value={phone}
            onChange={e => setPhone(e.target.value.replace(/[^\d\s]/g, ""))}
            className="flex-1 bg-transparent text-white font-bold text-sm placeholder:text-slate-500 focus:outline-none"
          />
        </div>
        <button
    ...
    <div className="space-y-4 pb-4">
      ...
      {depositLoading ? (
          ...
          <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.08)" }}>
            <div className="px-4 py-3 flex items-center justify-between" style={{ background: "rgba(255,255,255,0.06)" }}>
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">Dépôt Mobile Money</p>
                  <p className="text-white font-black text-sm">{selectedOp?.name}</p>
                </div>
              </div>
              <div className="rounded-lg px-2 py-1 bg-white/10">
            ...
            <div className="px-4 py-4" style={{ background: "rgba(255,255,255,0.03)" }}>
              ...
              <div className="flex items-center justify-between gap-3">
                <p className="text-white font-black text-2xl font-mono tracking-wide leading-none">
                  {depositInfo.depositNumber || "— — — — —"}
                </p>
                {depositInfo.depositNumber && (
                  <button onClick={copyDepositNumber}
                    className="flex items-center gap-1.5 rounded-xl px-3 py-2 font-bold text-xs transition-all flex-shrink-0"
            ...
            <div className="px-4 py-3 flex items-center justify-between" style={{ background: "rgba(255,255,255,0.03)", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
              <div>
                <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1">Montant exact</p>
                <p className="text-white font-black text-xl leading-none">
                  {depositInfo.amount?.toLocaleString("fr-FR")} <span className="text-slate-400 text-xs font-bold">FCFA</span>
                </p>
              </div>
              <div className="text-right">
                <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1">Depuis</p>
                <p className="text-slate-300 font-mono text-xs font-semibold">{fullPhone}</p>
              </div>
            </div>
          </div>

          {depositInfo.isInternational && depositInfo.internationalNote && (
            <div className="rounded-2xl p-3.5 flex gap-3" style={{ background: "rgba(245,158,11,0.1)", border: "1px solid rgba(245,158,11,0.25)" }}>
              <AlertTriangle size={15} className="text-amber-400 flex-shrink-0 mt-0.5" />
              <p className="text-xs text-amber-200">{depositInfo.internationalNote}</p>
            </div>
          )}

          {depositInfo.showInstruction && depositInfo.instruction && (
            <div className="rounded-2xl p-3.5 flex gap-3" style={{ background: "rgba(59,130,246,0.1)", border: "1px solid rgba(59,130,246,0.25)" }}>
              <Info size={14} className="text-blue-400 flex-shrink-0 mt-0.5" />
              <p className="text-blue-200 text-xs whitespace-pre-line leading-relaxed">{depositInfo.instruction}</p>
            </div>
          )}
          ...
          <div className="rounded-2xl p-4 space-y-4" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
            <div>
              ...
              <input
                ...
                className="w-full rounded-2xl px-4 py-3.5 text-sm font-semibold text-white placeholder:text-slate-500 focus:outline-none transition-colors"
            ...
            <div>
              ...
              <input
                ...
                className="w-full rounded-2xl px-4 py-3.5 text-sm font-semibold font-mono text-white placeholder:text-slate-500 focus:outline-none transition-colors"
                style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)" }}
              />
              <p className="text-slate-500 text-[10px] mt-1.5 pl-1">ID reçu par SMS après votre paiement</p>
            </div>
            <div>
              <p className="text-slate-400 text-[11px] font-black uppercase tracking-wider mb-2 flex items-center gap-1.5">
              ...
              {screenshotFile ? (
                <div className="relative rounded-2xl overflow-hidden" style={{ background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.25)" }}>
                  {screenshotFile.type.startsWith("image/") && screenshotPreview ? (
                    ...
                    <div className="flex flex-col items-center justify-center gap-2 py-8 px-4">
                      ...
                      <p className="text-white font-semibold text-sm text-center">{screenshotFile.name}</p>
                ...
                <button onClick={() => fileInputRef.current?.click()}
                  className="w-full rounded-2xl p-5 flex flex-col items-center gap-2.5 transition-colors"
                  ...
                  <p className="text-slate-300 text-sm font-semibold">Ajouter une capture d'écran</p>
        ...
        <div className="rounded-2xl p-4 text-center" style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.25)" }}>
          <p className="text-red-300 font-semibold text-sm">Impossible de charger les informations. Veuillez réessayer.</p>
        </div>
      )}
    </div>
  );
