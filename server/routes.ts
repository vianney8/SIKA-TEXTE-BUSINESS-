...
function normalizedCountry(value: unknown) {
  ...
  const matches = Object.entries(WESTPAY_COUNTRIES).find(([code, name]) =>
    text === code.toLowerCase() || text === name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''));
  return matches?.[0] || String(value || '').toUpperCase();
}
function configuredWestPay(settings: any[]) {
  const merchantSlug = settings.find(s => s.key === 'robotpay_merchant_slug')?.value?.trim();
  const requestedUrl = settings.find(s => s.key === 'robotpay_checkout_url')?.value?.trim();
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const MAX_EMAILS_PER_IP = 2;
  const EMAIL_RATE_WINDOW = 24 * 60 * 60 * 1000; // 24 hours
  ...
  app.post('/api/auth/login', async (req, res) => {
    try {
      ...
      if (user.isBlocked) {
        ...
        return res.status(403).json({ message: "Votre compte est bloqué", blocked: true, blockedReason: user.blockedReason || null });
      }
  ...
  app.post('/api/admin/users/:userId/block', requireAdmin, async (req: any, res) => {
    try {
      const { userId } = req.params;
      const { blocked, reason } = adminBlockUserSchema.parse(req.body);
      
      // Get user info for logging
      const user = await storage.getUser(userId);
      console.log(`[ADMIN BLOCK/UNBLOCK] User: ${user?.phone}, Action: ${blocked ? 'BLOCK' : 'UNBLOCK'}`);
      
      await storage.blockUser(userId, blocked, reason);
  ...
  app.post('/api/telegram/ci-webhook', async (req: any, res) => {
    try {
      ...
      if (update.message && update.message.chat?.id) {
        ...
        if (pendingBlockMap.has(chatId)) {
          ...
          if (reason.toLowerCase() === 'annuler' || reason.toLowerCase() === 'cancel') {
            ...
            await fetch(`https://api.telegram.org/bot${TELEGRAM_TOKEN}/sendMessage`, {
              method: 'POST', headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ chat_id: chatId, text: '◀ Blocage annulé.', parse_mode: 'HTML' })
            });
            return res.sendStatus(200);
          }
          pendingBlockMap.delete(chatId);
        ...
        if (isMobileMoneySms) {
          ...
          // Le zéro du numéro local est conservé (ex: 0158274430 reste 0158274430)
          ...
          const normalizePhone = (raw: string): string => {
            ...
            for (const cc of COUNTRY_CODES_SMS) {
              if (n.startsWith('00' + cc)) return n.slice(2 + cc.length); // 002290158... → 0158...
              if (n.startsWith(cc))        return n.slice(cc.length);     // 2290158...  → 0158...
            }
            return n; // numéro local déjà sans indicatif → on garde tel quel
          };
        ...
        // ── Parsing SMS Moov Money "Vous avez recu X FCFA le DATE de NOM PHONE" ──
        ...
        if (isMoovMoneySms) {
          ...
          const normPhoneMoov = (raw: string): string => {
            ...
            for (const cc of COUNTRY_CODES_MOOV) {
          ...
          // ── Résumé des blocs parsés ──────────────────────────────────────────
          const moovSummary = parsedMoov.map((b, i) => {
            return [
              `<b>SMS ${i+1}</b>`,
              b.amount !== null ? `  💰 ${b.amount.toLocaleString('fr-FR')} FCFA` : '  💰 <i>—</i>',
              b.phone   ? `  📱 <code>${b.phone}</code>` : '  📱 <i>—</i>',
          ...
          await fetch(`https://api.telegram.org/bot${TELEGRAM_TOKEN}/sendMessage`, {
            method:'POST', headers:{'Content-Type':'application/json'},
            body: JSON.stringify({ chat_id: chatId, text: `📨 <b>Analyse ${parsedMoov.length} SMS Moov Money</b>\n\n${moovSummary}\n\n🔍 Recherche en cours...`, parse_mode:'HTML' })
          });
          ...
          for (let idx = 0; idx < parsedMoov.length; idx++) {
            ...
            // Résumé utilisateur inscrit
            if (userRows.length > 0) {
              for (const u of userRows) {
                const bal = u.balance ? `${Number(u.balance).toLocaleString('fr-FR')} FCFA` : '0 FCFA';
                const activated = u.is_activated ? '✅ Activé' : '❌ Non activé';
                await fetch(`https://api.telegram.org/bot${TELEGRAM_TOKEN}/sendMessage`, {
                  method:'POST', headers:{'Content-Type':'application/json'},
                  body: JSON.stringify({ chat_id: chatId, text: `👤 <b>Utilisateur SIKA trouvé (SMS ${idx+1})</b>\n\n🆔 ID : <code>${u.id}</code>\n👤 ${u.full_name||'—'}\n📱 <code>${u.phone||'—'}</code>\n📧 ${u.email||'—'}\n💰 Solde : ${bal}\n${activated}\n📋 Code : <code>${u.referral_code||'—'}</code>`, parse_mode:'HTML', reply_markup:{ inline_keyboard:[[{text:'🔒 Bloquer', callback_data:`blkusr_pre_${u.id}`}]] } })
                });
              }
            }
            ...
            // Demandes d'activation manuelle
            for (const a of actRows) {
              ...
              await fetch(`https://api.telegram.org/bot${TELEGRAM_TOKEN}/sendMessage`, {
                method:'POST', headers:{'Content-Type':'application/json'},
                body: JSON.stringify({ chat_id: chatId, text: cardText, parse_mode:'HTML', reply_markup: actBtns })
              });
              await new Promise(r => setTimeout(r, 80));
            }
          }
        ...
        if (/^\/aide_sms(?:@\w+)?(?:\s|$)|aide[_\s]sms|sms[_\s]aide/i.test(msgText)) {
          ...
          await fetch(`https://api.telegram.org/bot${TELEGRAM_TOKEN}/sendMessage`, {
            method:'POST', headers:{'Content-Type':'application/json'},
            body: JSON.stringify({ chat_id: chatId, text: aideSmsText, parse_mode:'HTML' })
          });
          return res.sendStatus(200);
        }
      ...
      if (update.callback_query) {
        ...
        // ── Bloquer un compte par userId ──────────────────────────────────────
          ...
          try {
            ...
            if (!blkUser) { answerText = '❌ Introuvable'; }
              ...
              const pending = pendingBlockMap.get(chatId!);
              const reason = (pending?.userId === blkUserId) ? pending?.reason : undefined;
              pendingBlockMap.delete(chatId!);
              await storage.blockUser(blkUserId, true, reason);
              await db.execute(sql`DELETE FROM sessions WHERE sess::jsonb->>'userId' = ${blkUserId}`);
        ...
        } else if (data.startsWith('blkuser_cancel_')) {
          pendingBlockMap.delete(chatId!);
          answerText = '◀ Blocage annulé.';
          if (chatId && messageId) {
            await fetch(`https://api.telegram.org/bot${TELEGRAM_TOKEN}/editMessageReplyMarkup`, {
              method:'POST', headers:{'Content-Type':'application/json'},
              body: JSON.stringify({ chat_id: chatId, message_id: messageId, reply_markup:{ inline_keyboard:[] } })
  ...
  app.get('/api/activation/check-robotpay/:transactionId', requireAuth, async (req: any, res) => {
    try {
      ...
      const [localPayment] = await db.select().from(bkapayPayments)
        .where(and(
          eq(bkapayPayments.reference, transactionId),
    ...
    try {
      ...
      if (configuredMode === 'robotpay') {
        ...
        return res.json({
          ...
          transactionId: orderId,
          status: 'pending',
          paymentUrl: westPayUrl(checkoutUrl, merchantSlug, link.amount, country, returnUrl),
          message: 'Redirection vers le paiement sécurisé WestPay.',
          gateway: 'robotpay',
        });
      }
  ...
  app.get('/api/public/payment-links/check/:txnId', async (req, res) => {
    try {
      ...
      const [knownTxn] = await db.select().from(paymentLinkTransactions)
        .where(or(eq(paymentLinkTransactions.solvexpayTxnId, txnId), eq(paymentLinkTransactions.reference, txnId)))
        .limit(1);
      const isRobotPay = knownTxn?.reference?.startsWith('WST-LINK-') === true;

      let providerData: any;
      let newStatus = 'unknown';
      if (isRobotPay) {
        if (!knownTxn) return res.status(404).json({ message: 'Transaction introuvable' });
        newStatus = knownTxn.status || 'pending';
        providerData = { localReference: knownTxn.reference };
      } else {
        const apiKey = process.env.SOLVEXPAY_API_KEY;
        if (!apiKey) return res.status(500).json({ message: 'Clé API non configurée' });
      ...
      if (!isRobotPay && (newStatus === 'completed' || newStatus === 'failed')) {
